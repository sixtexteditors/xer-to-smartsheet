...
def _push_to_smartsheet(api_key: str, sheet_name: str, parsed: dict) -> str:
    ...
    if existing_id:
        ...
        for col_def in column_defs:
            if col_def["title"] not in col_map and not col_def.get("primary"):
                ...
                added = _with_retry(lambda c=col_obj: ss.Sheets.add_columns(sheet_id, [c]))
        ...
        result = _with_retry(lambda: ss.Home.create_sheet(new_sheet))
    ...
    for depth in sorted(wbs_by_depth.keys()):
        ...
        for parent_wbs_id, siblings in by_parent.items():
            ...
            for i in range(0, len(siblings), ADD_BATCH):
                batch = siblings[i:i + ADD_BATCH]
                rows = []
                for node in batch:
                ...
                result = _with_retry(
                    lambda r=_batch_rows: ss.Sheets.add_rows(sheet_id, r)
                )
    ...
    # --- Insert activity rows (flat, no hierarchy yet) ---
    task_id_to_ss_row_id = {}

    def _build_activity_row_flat(act):
    ...
    for i in range(0, len(activities_flat), ADD_BATCH):
        batch = activities_flat[i:i + ADD_BATCH]
        rows = [(act["_task_id"], _build_activity_row_flat(act)) for act in batch]
        _batch_rows = [r for _, r in rows]
        result = _with_retry(
            lambda r=_batch_rows: ss.Sheets.add_rows(sheet_id, r)
        )
    ...
    # --- Re-parent activity rows under their WBS nodes ---
    hierarchy_rows = []
    for act in activities_flat:
        tid = act["_task_id"]
        act_ss_id = task_id_to_ss_row_id.get(tid)
        parent_ss_id = wbs_id_to_ss_row_id.get(act["_wbs_id"])
        if act_ss_id is None or parent_ss_id is None:
            continue
        update_row = smartsheet.models.Row()
        update_row.id = act_ss_id
        update_row.parent_id = parent_ss_id
        hierarchy_rows.append(update_row)

    for i in range(0, len(hierarchy_rows), ADD_BATCH):
        _with_retry(
            lambda b=hierarchy_rows[i:i + ADD_BATCH]: ss.Sheets.update_rows(sheet_id, b)
        )

    # --- Populate predecessor / dependent link columns ---
    ...
    sheet_data = _with_retry(lambda: ss.Sheets.get_sheet(sheet_id))
    ...
    for act in activities_flat:
        ...
        if pred_parts:
            cells_to_update.append(make_cell("Predecessors", ",".join(pred_parts)))
        if pred_name_parts:
            cells_to_update.append(make_cell("Predecessor Names", ", ".join(pred_name_parts)[:4000]))
        ...
        if dep_parts:
            cells_to_update.append(make_cell("Dependent Row Numbers", ",".join(dep_parts)))
        if dep_name_parts:
            cells_to_update.append(make_cell("Dependent Names", ", ".join(dep_name_parts)[:4000]))

        if cells_to_update:
            update_row = smartsheet.models.Row()
            update_row.id = act_ss_row_id
            update_row.cells = cells_to_update
            link_rows.append(update_row)

    for i in range(0, len(link_rows), ADD_BATCH):
        _with_retry(
            lambda b=link_rows[i:i + ADD_BATCH]: ss.Sheets.update_rows(sheet_id, b)
        )
